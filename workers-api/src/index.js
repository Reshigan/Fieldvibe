...
app.get('/api/agent/dashboard', authMiddleware, async (c) => {
  try {
    ...
    // Per-company registration counts
    let perCompanyRegs = { results: [] };
    try {
      const agentCompanyIds = (companies.results || []).map(co => co.id);
      if (agentCompanyIds.length > 0) {
        const regPh = agentCompanyIds.map(() => '?').join(',');
        perCompanyRegs = await db.prepare(`SELECT company_id, COUNT(*) as today_count, SUM(CASE WHEN DATE(created_at) >= ? THEN 1 ELSE 0 END) as month_count FROM individual_registrations WHERE tenant_id = ? AND agent_id = ? AND company_id IN (${regPh}) AND DATE(created_at) >= ? GROUP BY company_id`).bind(monthStart, tenantId, userId, ...agentCompanyIds, today).all();
      }
    } catch { /* */ }
    // Build a quick lookup for per-company today/month regs
    let perCompanyRegsTodayAll = { results: [] };
    try {
      const agentCompanyIds2 = (companies.results || []).map(co => co.id);
      if (agentCompanyIds2.length > 0) {
        const regPh2 = agentCompanyIds2.map(() => '?').join(',');
        perCompanyRegsTodayAll = await db.prepare(`SELECT company_id, SUM(CASE WHEN DATE(created_at) = ? THEN 1 ELSE 0 END) as today_count, COUNT(*) as month_count FROM individual_registrations WHERE tenant_id = ? AND agent_id = ? AND company_id IN (${regPh2}) AND DATE(created_at) >= ? GROUP BY company_id`).bind(today, tenantId, userId, ...agentCompanyIds2, monthStart).all();
      }
    } catch { /* */ }
    ...
    // Resolve working calendar per company for accurate monthly targets
    const currentMonth = today.substring(0, 7);
    const companyWdCache = {};
    for (const ctr of companyTargetRules) {
      const wdConfig = await getEffectiveWorkingDaysConfig(db, tenantId, userId, ctr.company_id);
      companyWdCache[ctr.company_id] = {
        perWeek: [wdConfig.monday, wdConfig.tuesday, wdConfig.wednesday, wdConfig.thursday, wdConfig.friday, wdConfig.saturday, wdConfig.sunday].filter(Boolean).length || 5,
        perMonth: countWorkingDaysInMonth(wdConfig, currentMonth),
      };
    }
    ...
    const companyTargets = companyTargetRules.map(ctr => {
      ...
      weekTargetVisits += dayTarget * wd.perWeek;
      weekTargetRegs += dayRegTarget * wd.perWeek;
      monthTargetVisits += dayTarget * wd.perMonth;
      monthTargetRegs += dayRegTarget * wd.perMonth;
      return {
        company_id: ctr.company_id,
        company_name: ctr.company_name,
        // Daily targets
        ...
        // Working days info
        working_days_per_week: wd.perWeek,
        working_days_per_month: wd.perMonth,
      };
    });
...
app.get('/api/agent/performance', authMiddleware, async (c) => {
  try {
    ...
    if (teamLeadId) {
      ...
      let teamActualVisits = totalTeamVisits;
      let teamTargetRegs = (agentTargets?.target_registrations || 0) + (tlOwnTargets?.target_registrations || 0);
      let teamActualRegs = totalTeamRegs;

      // Fallback: if monthly_targets yields 0 targets, use company_target_rules
      if (teamTargetVisits === 0) {
        const allTeamUserIds = [...(teamMembers?.results || []).map(m => m.id), teamLeadId];
        const ruleTotals = await computeTargetTotalsFromRules(db, tenantId, allTeamUserIds, perfMonthStart);
        ...
        if (mgrTlIds.length > 0) {
          ...
          if (perfAllMgrUserIds.length > 0) {
            ...
            const [perfMgrTargets, perfMgrLiveVisits] = await Promise.all([
              ...
              db.prepare(`SELECT COUNT(*) as count FROM visits WHERE tenant_id = ? AND agent_id IN (${perfAllPh}) AND visit_date >= ?`).bind(tenantId, ...perfAllMgrUserIds, perfMonthStart).first(),
            ]);
    ...
    if (targets.length === 0) {
      ...
      for (const t of targets) {
        ...
        try {
          const liveVisits = await db.prepare("SELECT COUNT(*) as count FROM visits WHERE agent_id = ? AND tenant_id = ? AND visit_date >= ?" + companyFilter).bind(userId, tenantId, monthStartDate, ...companyBinds).first();
          const liveRegs = await db.prepare("SELECT COUNT(*) as count FROM individual_registrations WHERE agent_id = ? AND tenant_id = ? AND created_at >= ?" + companyFilter).bind(userId, tenantId, monthStartDate + ' 00:00:00', ...companyBinds).first();
          const liveConvs = await db.prepare("SELECT COUNT(*) as count FROM individual_registrations WHERE agent_id = ? AND tenant_id = ? AND converted = 1 AND conversion_date >= ?" + companyFilter).bind(userId, tenantId, monthStartDate, ...companyBinds).first();
          // Also get per-type breakdown
          let storeVisits = 0, individualVisits = 0;
          try {
            const typeBreakdown = await db.prepare("SELECT visit_type, COUNT(*) as count FROM visits WHERE agent_id = ? AND tenant_id = ? AND visit_date >= ?" + companyFilter + " GROUP BY visit_type").bind(userId, tenantId, monthStartDate, ...companyBinds).all();
...
app.get('/api/team-lead/dashboard', authMiddleware, async (c) => {
  try {
    ...
    if (memberIds.length === 0) {
      ...
      // Fallback: if monthly_targets yields 0 targets, use company_target_rules
      if (tlTV === 0) {
        const tlRuleTargets = await generateTargetsFromRules(db, tenantId, userId, currentMonth + '-01');
...
app.get('/api/manager/dashboard', authMiddleware, async (c) => {
  try {
    ...
    for (const tl of (teamLeads.results || [])) {
      ...
      // Fallback: if monthly_targets yields 0, use company_target_rules for agents
      ...
      let tlOwnTVMgr = tlOwnTgt?.tv || 0;
      // Fallback: if monthly_targets yields 0 for TL, use company_target_rules
      if (tlOwnTVMgr === 0) {
        const tlRuleTargets = await generateTargetsFromRules(db, tenantId, tl.id, currentMonth + '-01');
        tlOwnTVMgr = tlRuleTargets.reduce((s, t) => s + (t.target_visits || 0), 0);
      }
      teamTargetVisits += tlOwnTVMgr;
      ...
      teamsData.push({
        ...
        actual_registrations: teamActualRegs,
        achievement: teamTargetVisits > 0 ? Math.round((teamActualVisits / teamTargetVisits) * 100) : 0,
        team_lead_own: { target_visits: tlOwnTVMgr, actual_visits: tlOwnVC?.count || 0, target_registrations: tlOwnTgt?.tr || 0, actual_registrations: tlOwnRC?.count || 0 },
      });
    }
...
api.get('/admin/territories', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
api.post('/admin/territories', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
  const id = crypto.randomUUID();
...
api.put('/admin/territories/:id', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  const body = await c.req.json();
...
api.delete('/admin/territories/:id', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
...
api.get('/admin/campaigns', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const campaigns = await db.prepare('SELECT * FROM campaigns WHERE tenant_id = ? ORDER BY created_at DESC').bind(tenantId).all();
...
api.post('/admin/campaigns', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const userId = c.get('userId');
  const body = await c.req.json();
...
api.put('/admin/campaigns/:id', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  const body = await c.req.json();
...
api.delete('/admin/campaigns/:id', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  await db.prepare('DELETE FROM campaigns WHERE id = ? AND tenant_id = ?').bind(id, tenantId).run();
...
api.get('/admin/commission-rules', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
api.get('/admin/boards', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
api.get('/admin/audit-logs', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const { page = '1', limit = '50' } = c.req.query();
  const offset = (parseInt(page) - 1) * parseInt(limit);
...
api.get('/admin/pos-library', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
api.get('/roles', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
api.get('/field-ops/reports/kpis', authMiddleware, async (c) => {
  try {
    ...
    const conversions = await db.prepare(`SELECT COUNT(*) as count FROM visits v WHERE v.tenant_id = ? AND v.status = 'completed' AND v.visit_type = 'individual'${dateFilter}`).bind(...binds).first();
  ...
  try {
    ...
    const agents = await db.prepare(`
      SELECT v.agent_id, u.first_name || ' ' || u.last_name as agent_name,
        ...
        SUM(CASE WHEN v.status = 'completed' AND v.visit_type = 'individual' THEN 1 ELSE 0 END) as conversions
      FROM visits v
...
api.get('/field-ops/reports/conversion-stats', authMiddleware, async (c) => {
  try {
    ...
    const converted = await db.prepare(`SELECT COUNT(*) as count FROM visits WHERE tenant_id = ? AND status = 'completed' AND visit_type = 'individual'${dateFilter}`).bind(...binds).first();
  ...
  try {
    ...
    const checkins = await db.prepare(`
      SELECT v.id, v.visit_date as timestamp, v.status, v.agent_id,
        CASE WHEN v.visit_type = 'individual' AND v.status = 'completed' THEN 1 ELSE 0 END as converted,
    ...
    const stats = await db.prepare(`
      SELECT COUNT(*) as total_checkins,
        ...
        SUM(CASE WHEN status = 'completed' AND visit_type = 'individual' THEN 1 ELSE 0 END) as conversions
  ...
  try {
    ...
    const statsResult = await db.prepare(`
      SELECT COUNT(*) as total_customers,
        SUM(CASE WHEN status = 'completed' AND visit_type = 'individual' THEN 1 ELSE 0 END) as converted,
        SUM(CASE WHEN visit_type = 'store' THEN 1 ELSE 0 END) as already_betting
