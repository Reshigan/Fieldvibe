...
export default function QuickVisitPage() {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Start a visit in under 30 seconds</p>
        ...
        {[1, 2, 3].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
              s === step ? 'bg-[#00E87B] text-black' : s < step ? 'bg-green-800 text-green-200' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'
            }`}>
              {s < step ? '\u2713' : s}
            </div>
            {s < 3 && <div className={`w-8 h-0.5 ${s < step ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-700'}`} />}
          </div>
        ))}
      </div>
      ...
      {step === 1 && (
        <div>
          ...
          {loadingNearby ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">Finding nearby customers...</div>
          ) : nearby.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">No nearby customers found. Check your location settings.</div>
          ) : (
              ...
              {nearby.map(c => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg border transition-colors ${
                    selectedCustomer === c.id ? 'border-[#00E87B] bg-[#00E87B]/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-500'
                  }`}
                  aria-label={`Select ${c.name}`}
                >
                  <div className="font-medium">{c.name}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">{c.address} - {c.distance_km?.toFixed(1)}km away</div>
                </button>
              ))}
            </div>
          )}
      ...
      {step === 2 && (
        <div>
            ...
            {purposes.map(p => (
              <button
                ...
                className={`p-3 rounded-lg border text-center transition-colors ${
                  purpose === p.value ? 'border-[#00E87B] bg-[#00E87B]/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-500'
                }`}
          ...
          <button onClick={() => setStep(1)} className="mt-3 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">Back</button>
        </div>
      )}
      ...
      {step === 3 && (
        <div>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-4 border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 mt-2">Purpose</div>
            <div className="font-medium">{purposes.find(p => p.value === purpose)?.label}</div>
            {position?.latitude && position?.longitude && (
              <>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-2">Location</div>
          ...
          <button onClick={() => setStep(2)} className="mt-3 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white block mx-auto">Back</button>
        </div>
      )}
    </div>
  )
